...
{
MeshManager::MeshManager(std::shared_ptr<Context> context) : Manager<Mesh, MeshResource>(context)
{
...
void MeshManager::ProcessPendingUpdateResources(RenderContext renderContext)
{

    auto meshResourcesToUpdate = ToVector(mPendingUpdateResources);
    std::vector<vk::BufferMemoryBarrier2> bufferBarriers{};
    bufferBarriers.reserve(meshResourcesToUpdate.size());
    for (auto &meshResource : meshResourcesToUpdate)
    {
        auto mesh = static_cast<Mesh *>(meshResource->mOwnerAsset);
        if (!meshResource->mStagingVertexBuffer || !meshResource->mStagingIndexBuffer)
        {
        ...
        if (!mesh->mDynamic)
        {
            ...
            //     meshResource->ReleaseStaging(context);
            // });
        }
        vk::BufferCopy2 vertexCopyRegion{}, indexCopyRegion{};
        vk::CopyBufferInfo2 copyVertexBufferInfo{}, copyIndexBufferInfo{};
        vertexCopyRegion.setSize(mesh->mVertices.size() * sizeof(Vertex)).setSrcOffset(0).setDstOffset(0);
        indexCopyRegion.setSize(mesh->mIndices.size() * sizeof(uint32_t)).setSrcOffset(0).setDstOffset(0);
        copyVertexBufferInfo.setSrcBuffer(meshResource->mStagingVertexBuffer)
            .setDstBuffer(meshResource->mVertexBuffer)
            .setRegions(vertexCopyRegion);
        copyIndexBufferInfo.setSrcBuffer(meshResource->mStagingIndexBuffer)
            .setDstBuffer(meshResource->mIndexBuffer)
            .setRegions(indexCopyRegion);
        renderContext.CommandBuffer.copyBuffer2(copyVertexBufferInfo);
        renderContext.CommandBuffer.copyBuffer2(copyIndexBufferInfo);
        vk::BufferMemoryBarrier2 vertexBufferBarrier{}, indexBufferBarrier{};
        vertexBufferBarrier.setSrcStageMask(vk::PipelineStageFlagBits2::eCopy)
            .setDstStageMask(vk::PipelineStageFlagBits2::eVertexInput)
            .setSrcAccessMask(vk::AccessFlagBits2::eTransferWrite)
            .setDstAccessMask(vk::AccessFlagBits2::eVertexAttributeRead)
            .setBuffer(meshResource->mVertexBuffer)
            .setOffset(0)
            .setSize(mesh->mVertices.size() * sizeof(Vertex));
        indexBufferBarrier.setSrcStageMask(vk::PipelineStageFlagBits2::eCopy)
            .setDstStageMask(vk::PipelineStageFlagBits2::eVertexInput)
            .setSrcAccessMask(vk::AccessFlagBits2::eTransferWrite)
            .setDstAccessMask(vk::AccessFlagBits2::eIndexRead)
            .setBuffer(meshResource->mIndexBuffer)
            .setOffset(0)
            .setSize(mesh->mIndices.size() * sizeof(uint32_t));
        bufferBarriers.push_back(vertexBufferBarrier);
        bufferBarriers.push_back(indexBufferBarrier);
    }
    vk::DependencyInfo dependencyInfo{};
    dependencyInfo.setBufferMemoryBarriers(bufferBarriers);
    renderContext.CommandBuffer.pipelineBarrier2(dependencyInfo);
}
std::shared_ptr<Mesh> MeshManager::CreateTriangle()
{
    ...
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::Triangle, vertices, indices);
...
std::shared_ptr<Mesh> MeshManager::CreateCube()
{
    ...
    const std::vector<uint32_t> indices = {// 正面（-Z）
                                           ...
                                           // 底面(-Y)
                                           20, 22, 21, 22, 23, 21};
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::Cube, vertices, indices);
    LogInfo("Created「cube」mesh");
    return mesh;
}
std::shared_ptr<Mesh> MeshManager::CreateSphere()
{
    ...
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::Sphere, vertices, indices);
    LogInfo("Created「sphere」mesh");
    return mesh;
}
std::shared_ptr<Mesh> MeshManager::CreatePlane()
{
    // // 平面顶点
    // const std::vector<Vertex> vertices = {
    //     // 顶面（Z+）
    ...
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::Plane, vertices, indices);
    LogInfo("Created「plane」mesh");
    return mesh;
}
std::shared_ptr<Mesh> MeshManager::CreateCylinder()
{
    ...
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::Cylinder, vertices, indices);
    LogInfo("Created「cylinder」mesh");
    return mesh;
}
std::shared_ptr<Mesh> MeshManager::CreateSky()
{
    const std::vector<Vertex> vertices = {
        // 前面
        {{-1.0f, -1.0f, 1.0f}, {}, {}},
    ...
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::Sky, vertices, indices);
    LogInfo("Created「sky」mesh");
    return mesh;
}
std::shared_ptr<Mesh> MeshManager::CreateFullscreenTriangle()
{
    const std::vector<Vertex> vertices = {
        // 全屏三角形顶点
    ...
    auto mesh = std::make_unique<Mesh>(DefaultMeshType::FullscreenTriangle, vertices, indices);
