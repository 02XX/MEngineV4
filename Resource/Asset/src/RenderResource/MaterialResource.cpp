...
void MaterialResource::InitStaging(std::shared_ptr<Context> context, vk::DeviceSize bufferSize)
{
    // Staging Buffer
...
void MaterialResource::ReleaseStaging(std::shared_ptr<Context> context)
{
...
void MaterialResource::UploadData()
{
    auto material = static_cast<Material *>(mOwnerAsset);
    uint8_t *target = reinterpret_cast<uint8_t *>(mStagingBufferAllocationInfo.pMappedData);
    material->UpdateMaterialData(target);
}
void MaterialResource::Bind(vk::CommandBuffer commandBuffer, vk::PipelineLayout pipelineLayout, vk::Pipeline pipeline)
{
    vk::BindDescriptorSetsInfo bindInfo{};
    bindInfo.setDescriptorSets(mDescriptorSet).setFirstSet(2).setLayout(pipelineLayout);
